...
class NtfyAPI:
        ...
        while True:
            try:
                ...
                async with httpx.AsyncClient(timeout=None) as client:
                    logger.info(
                        f"Starting NTFY response listener for topic: {self.topic}"
                    )
            ...
            if event_type == "message":
                ...
                try:
                    ...
                    if message.strip().startswith("{") and message.strip().endswith(
                        ...
                        if "id" in json_data and (
                            "response" in json_data or "text" in json_data
                        ):
                            logger.info(f"Detected text input response: {json_data}")
                            message_id = json_data.get("id")
                            response_text = json_data.get(
                                "response", json_data.get("text", "")
                            )
    ...
    async def ask_team_info(
        ...
        if not self.enabled:
            logger.warning(
                "NTFY integration not enabled - cannot send team information notifications"
            )
        ...
        if missing_fields:
            ...
            logger.info(
                "All team information fields are populated, no notification needed"
            )

        return team_info

    async def ask_resolve_game_conflict(
        self,
        combined_video_path: str,
        group_dir: str,
        ...
        if not self.enabled:
            logger.warning(
                "NTFY integration not enabled - cannot send game conflict resolution notification"
            )
            return None

        if not game_options or len(game_options) < 2:
            logger.warning(
                f"Not enough game options to resolve conflict: {len(game_options) if game_options else 0}"
            )
        ...
        for i, game in enumerate(game_options):
            ...
            opponent = game.get(
                "opponent_name", game.get("opponent", "Unknown Opponent")
            )
        ...
        try:
            ...
            if selected_index is not None and 0 <= selected_index < len(game_options):
                logger.info(
                    f"User selected game option {selected_index + 1}: {game_options[selected_index]}"
                )
                return game_options[selected_index]
            else:
                logger.warning(
                    f"Could not determine selected game from response: {response_text}"
                )
                return None
