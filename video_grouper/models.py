...
from typing import Optional, Any, TypedDict, Tuple, Union, TYPE_CHECKING
import logging
import configparser
import os
import re
...
class MatchInfo:
    ...
    def get_or_create(
        ...
        if not os.path.exists(match_info_path):
            ...
            source_dist_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "match_info.ini.dist"
            )
    ...
    def get_total_duration_seconds(self) -> int:
        ...
        try:
            ...
            if len(parts) == 2:
                ...
                logger.warning(
                    f"Invalid time format: {self.total_duration}, using default of 90 minutes"
                )
                return 90 * 60  # Default to 90 minutes
        except (ValueError, TypeError) as e:
            logger.warning(
                f"Error parsing duration '{self.total_duration}': {e}, using default of 90 minutes"
            )
            return 90 * 60  # Default to 90 minutes
    ...
    def get_start_offset(self) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Error parsing start offset '{self.start_time_offset}': {e}"
            )
            return ""
    ...
    def get_sanitized_names(self) -> Tuple[str, str, str]:
        ...
        opponent_sanitized = re.sub(
            r"[^a-zA-Z0-9]", "", self.opponent_team_name
        ).lower()
...
try:
    ...
    def create_ffmpeg_task(
        task_type: str, item_path: str, match_info: Optional[MatchInfo] = None
    ) -> Optional[FFmpegTask]:
    ...
    class FFmpegTask:
        ...
        def __eq__(self, other):
            ...
            if not isinstance(other, FFmpegTask):
                return False
            return (
                self.task_type == other.task_type and self.item_path == other.item_path
            )

        def to_dict(self) -> dict:
            """Convert task to a dictionary for serialization."""
            return {"task_type": self.task_type, "item_path": self.item_path}
    ...
    class TrimTask(FFmpegTask):
        ...
        def __eq__(self, other):
            ...
            if not isinstance(other, TrimTask):
                return False
            return super().__eq__(other) and self.match_info == other.match_info
        ...
        for line in response_text.strip().split("\n"):
            ...
            try:
                ...
                start_time = datetime.strptime(
                    parts.get("startTime", ""), "%Y-%m-%d %H:%M:%S"
                )
                end_time = datetime.strptime(
                    parts.get("endTime", ""), "%Y-%m-%d %H:%M:%S"
                )
                ...
                metadata = {
                    ...
                    if k not in ["path", "startTime", "endTime"]
                }
