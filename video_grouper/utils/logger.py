...
def setup_logging_from_config(config) -> None:
    ...
    log_file = None
    storage_path = (
        getattr(config.storage, "path", None) if hasattr(config, "storage") else None
    )
    ...
    app_name = (
        getattr(config.logging, "app_name", "video_grouper")
        if hasattr(config, "logging")
        else "video_grouper"
    )
    backup_count = (
        getattr(config.logging, "backup_count", 30)
        if hasattr(config, "logging")
        else 30
    )
    level = (
