...
class MatchInfoItemWidget(QWidget):
    ...
    """
    def __init__(self, group_dir_path, refresh_callback, timezone_str="UTC", parent=None):
    ...
    def populate_thumbnails(self):
        col, row = 0, 0
        thumbnail_files = [f for f in os.listdir(self.group_dir_path) if f.lower().endswith('.jpg')]
        for filename in sorted(thumbnail_files):
            ...
            thumb_label.mousePressEvent = lambda event, p=pixmap: self.show_image_preview(p)
