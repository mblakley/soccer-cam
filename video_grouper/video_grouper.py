...
def trim_file_to_offset(combined_filename: str, match_info: dict):
    ...
    output_dir = f"{dir_date} - vs {str(match_info['opponent_team_name'])} ({str(match_info['location'])})"
    output_dir_path = os.path.join(directory_path, output_dir)
    os.makedirs(output_dir_path, exist_ok=True)
    output_file = os.path.join(directory_path, output_dir, f"{match_info['my_team_name'].lower().replace(' ', '')}-{match_info['opponent_team_name'].lower().replace(' ', '')}-{match_info['location'].lower().replace(' ', '')}-{formatted_date}-raw.mp4")
...
async def make_http_request(url: str, auth: httpx.DigestAuth):
    async with httpx.AsyncClient() as client:
        return await client.get(url, auth)

async def find_files_to_download(auth: httpx.DigestAuth):
            ...
            if (response.status_code == 200):
                ...
                for line in response.text.split("\n"):
                    ...
                    if (".FilePath" in line):
                        ...
                        response = await make_http_request(loadfile_url, auth=auth)
