        ...
        try:
            ...
            if queue_type == QueueType.VIDEO:
                if self._video_processor:
                    ...
                    logger.info(f"QUEUE: Queued {task_type} task to video processor: {task}")
                    return True
                else:
                    logger.error(f"QUEUE: No video processor available for {task_type} task: {task}")
                    return False
            
            elif queue_type == QueueType.UPLOAD:
                if self._upload_processor:
                    await self._upload_processor.add_item(task)
                    logger.info(f"QUEUE: Queued {task_type} task to upload processor: {task}")
                    return True
                else:
                    logger.error(f"QUEUE: No upload processor available for {task_type} task: {task}")
                    return False
            
            elif queue_type == QueueType.DOWNLOAD:
                if self._download_processor:
                    await self._download_processor.add_item(task)
                    logger.info(f"QUEUE: Queued {task_type} task to download processor: {task}")
                    return True
                else:
                    logger.error(f"QUEUE: No download processor available for {task_type} task: {task}")
                    return False
            
            else:
                logger.error(f"QUEUE: Unknown queue type: {queue_type} for task: {task}")
                return False
