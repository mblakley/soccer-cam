...
class CameraPoller(PollingProcessor):
    ...
    def __init__(self, storage_path: str, config: Any, camera: Any, poll_interval: int = 60):
    ...
    async def _sync_files_from_camera(self) -> None:
        ...
        logger.info(f"CAMERA_POLLER: Looking for new files from: {start_time} to {end_time}")
        
        files = await self.camera.get_file_list(start_time=start_time, end_time=end_time)
        
        if not files:
            logger.debug("CAMERA_POLLER: No new files found on the camera since last sync.")
            return
            
        logger.info(f"CAMERA_POLLER: Found {len(files)} new files to process.")
        existing_dirs = [os.path.join(self.storage_path, d) for d in os.listdir(self.storage_path) 
        ...
        for file_info in files:
            try:
                ...
                if connected_timeframes:
                    ...
                    for frame_start, frame_end in connected_timeframes:
                        ...
                        if file_start_utc < frame_end_or_now and file_end_utc > frame_start:
                            logger.info(f"CAMERA_POLLER: Skipping file {filename} as it overlaps with connected timeframe from {frame_start} to {frame_end_or_now}")
                            should_skip = True
                            break
                ...
                if dir_state.is_file_in_state(local_path):
                    logger.debug(f"CAMERA_POLLER: File {filename} is already known. Skipping.")
                    continue
                ...
                if not recording_file.skip and self.download_processor:
                    ...
                    logger.info(f"CAMERA_POLLER: Skipping download for {os.path.basename(local_path)} as per state file.")

            except Exception as e:
                logger.error(f"CAMERA_POLLER: Error processing file info {file_info}: {e}")
        
        if latest_end_time:
            await self._update_latest_processed_time(latest_end_time)
            logger.info(f"CAMERA_POLLER: File sync complete. New high-water mark set to: {latest_end_time}")
    ...
    async def _get_latest_processed_time(self) -> Optional[datetime]:
        ...
        try:
            ...
            logger.error(f"CAMERA_POLLER: Could not read or parse latest video file timestamp: {e}")
            return None
    ...
    async def _update_latest_processed_time(self, timestamp: datetime):
        ...
        try:
            ...
            logger.debug(f"CAMERA_POLLER: Updated latest processed time to: {timestamp}")
        except Exception as e:
