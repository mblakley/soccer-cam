...
class ConvertTask(BaseFfmpegTask):
    ...
    async def _handle_post_conversion_actions(
        self, queue_task: Optional[Callable[[Any], Awaitable[None]]] = None
    ) -> None:
        ...
        try:
            ...
            logger.info(
                f"Post-conversion: Checking status for group {os.path.basename(group_dir)}"
            )
            ...
            if dir_state.is_ready_for_combining():
                logger.info(
                    f"CONVERT: Group {os.path.basename(group_dir)} is ready for combining."
                )
                ...
                if queue_task:
                    ...
                    logger.warning(
                        f"CONVERT: No task queue function available to queue combine task for {group_dir}"
                    )
    ...
    async def _handle_task_failure(self) -> None:
        ...
        try:
            ...
            await dir_state.update_file_state(
                self.file_path, status="conversion_failed"
            )
        except Exception as e:
                ...
                if os.path.exists(mp4_path) and os.path.getsize(mp4_path) > 0:
                    try:
                        ...
                        if mp4_duration and mp4_duration > 0:
                            ...
                            deleted_count += 1
                            logger.info(
                                f"CONVERT: Removed orphaned DAV file: {dav_path}"
                            )
                    except Exception as e:
                        logger.error(
                            f"CONVERT: Error processing file {dav_path} for cleanup: {e}"
                        )
        except Exception as e:
            logger.error(
                f"CONVERT: Error processing directory {directory} for cleanup: {e}"
            )
    ...
    def serialize(self) -> Dict[str, Any]:
        ...
        """
        return {"task_type": self.task_type, "file_path": self.file_path}
