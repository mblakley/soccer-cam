...
class TaskRegistry:
    ...
    def register_task(self, task_class: Type[BaseTask]) -> None:
        ...
        if isinstance(queue_type, property):
            logger.error(f"Task class {task_class.__name__} defines queue_type as a property. It must be a class attribute or @classmethod.")
            return
        ...
        if not task_type:
            logger.warning(f"Task class {task_class.__name__} has no task_type property")
            return
            
        if not queue_type:
            logger.warning(f"Task class {task_class.__name__} has no queue_type property or classmethod")
            return
        ...
        logger.debug(f"Registered task type '{task_type}' for queue '{queue_type.value}'")
    
    def get_task_class(self, task_type: str, queue_type: Optional[QueueType] = None) -> Optional[Type[BaseTask]]:
        """
    ...
    def deserialize_task(self, data: Dict[str, object], queue_type: Optional[QueueType] = None) -> Optional[BaseTask]:
        """
        ...
        try:
            ...
            if not task_class:
                logger.error(f"Unknown task type '{task_type}' for queue type {queue_type}")
                return None
