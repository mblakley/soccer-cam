"""
Recording file model for camera file tracking.
"""
...
class RecordingFile:
    ...
    def __str__(self) -> str:
        ...
        filename = (
            self.file_path.split("/")[-1] if "/" in self.file_path else self.file_path
        )
        time_range = f"{self.start_time.strftime('%H:%M:%S')}-{self.end_time.strftime('%H:%M:%S')}"
        return f"RecordingFile({filename}, {time_range}, {self.status})"
