...
def test_system_tray_icon_initialization(
    mock_start_update_checker, mock_init_ui, mock_super_init, mock_load_config,
    mock_get_shared_data_path, mock_get_full_version, mock_get_version,
    mock_config, tmp_path, qapp
):
...
def test_start_service_success(mock_start_service):
    ...
    with patch('video_grouper.tray.main.SystemTrayIcon.__init__', lambda x: None):
        ...
        tray_icon.showMessage.assert_called_once_with('Service', 'Service started successfully')
