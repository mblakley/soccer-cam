# Deploy pipeline worker to a remote machine.
# Run from the SERVER in an elevated PowerShell prompt.
#
# Usage:
#   .\training\worker\deploy_worker.ps1 -Machine worker1
#
# What it does:
...
$RemoteUser = "training"
$RemotePass = "changeme"  # Set the remote user's password

$Machines = @{
    # Add entries for each worker machine:
    # "worker1" = @{
...
$ProjectDir = $PSScriptRoot | Split-Path | Split-Path  # repo root
