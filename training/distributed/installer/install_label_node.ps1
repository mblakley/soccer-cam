...
if ($ModelSource -ne "") {
    ...
    Copy-Item -Path $ModelSource -Destination "$installDir\models\balldet_fp16.onnx" -Force
} else {
    Write-Host "WARNING: No model source specified. Copy balldet_fp16.onnx to $installDir\models\ manually." -ForegroundColor Red
}
...
foreach ($gameId in $gamesToProcess) {
    ...
    $config = @{
        video_dir = $videoDir
        model = "$installDir\models\balldet_fp16.onnx"
