#!/bin/bash
# Build a patched .pak that:
#   - Carries the daily-driver patches (HTTP /downloadfile/ unlock + bitrate cap)
#   - Bakes a "record-at-home gate" into /etc/init.d/start_app so the camera
#     does NOT record when it boots on the home network, and records everywhere
#     else (the field, no network, any failure). The home gateway MAC(s) are
#     baked in at build time.
#
# How it differs from build_netstate.sh:
#   - The netstate daemon (S99) toggles recording via the HTTP API AFTER boot,
#     which is too late to stop the first ~4s boot segment. This gate decides
#     BEFORE ./recorder is launched, so no home boot stub is written.
#   - It needs no camera credentials (it reads /proc/net/route + /proc/net/arp,
#     not the API), so it takes no user/pass.
#
# FAIL-ENABLED by construction: the recorder is started unless a home gateway
# MAC is positively confirmed within the timeout. See docs/RECORD_GATE_DESIGN.md.
#
# Usage:
#   sudo bash build_recordgate.sh <stock.pak> <output.pak> <kbps> <home_mac> [more_macs...]
# Example:
#   sudo bash build_recordgate.sh stock.pak duo3_recordgate.pak 20480 aa:bb:cc:dd:ee:ff
#
# To find your home gateway MAC (run on the home LAN):
#   ip route | awk '$1=="default" {print "gateway:", $3}'   # gateway IP
#   arp -n          # find that gateway IP's MAC  (Windows: arp -a)
#
# UNVERIFIED: not yet flashed/validated on a camera. Bench-test against the
# fail-enabled matrix in docs/RECORD_GATE_DESIGN.md before any field use.
set -euo pipefail

STOCK="${1:?usage: $0 <stock.pak> <output.pak> <kbps> <home_mac> [more_macs...]}"
OUT="${2:?usage}"
KBPS="${3:?usage}"
shift 3
HOME_MACS="$*"
[[ -n "$HOME_MACS" ]] || { echo "ERROR: at least one home MAC required"; exit 1; }
[[ "$EUID" -eq 0 ]] || { echo "ERROR: run as root"; exit 1; }

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PAK_DIR="$(cd "$HERE/../pak" && pwd)"
GATE_TEMPLATE="$HERE/../runtime/recordgate/start_app_gate.template"
[[ -f "$GATE_TEMPLATE" ]] || { echo "ERROR: gate template not found at $GATE_TEMPLATE"; exit 1; }
WORK="$(mktemp -d)"
trap "rm -rf '$WORK'" EXIT
cd "$WORK"

HOME_MACS_LC=$(echo "$HOME_MACS" | tr 'A-Z' 'a-z')

echo "Target: HTTP unlock + bitrate=${KBPS} + record-gate(home_macs=$HOME_MACS_LC)"
echo

echo "==> 1) Extract app section"
python3 - <<PY
import struct
data = open("$STOCK","rb").read()
base = 0x18 + 7*0x48
off = struct.unpack("<Q", data[base+0x38:base+0x40])[0]
sz  = struct.unpack("<Q", data[base+0x40:base+0x48])[0]
open("app_stock.bin","wb").write(data[off:off+sz])
PY

echo "==> 2) Extract rootfs section"
python3 - <<PY
import struct
data = open("$STOCK","rb").read()
base = 0x18 + 5*0x48     # section index 5 = rootfs
off = struct.unpack("<Q", data[base+0x38:base+0x40])[0]
sz  = struct.unpack("<Q", data[base+0x40:base+0x48])[0]
open("rootfs_stock.bin","wb").write(data[off:off+sz])
print(f"  rootfs orig size: {sz} bytes")
PY

echo "==> 3) Unsquashfs both"
unsquashfs -d app_unpacked  -no-progress app_stock.bin  >/dev/null
unsquashfs -d rootfs_unpacked -no-progress rootfs_stock.bin >/dev/null

echo "==> 4) Carry the /downloadfile/ HTTP unlock (in 'device' inside app)"
python3 - <<'PY'
SRC = (b"location /downloadfile/ {\n"
       b"            internal;\n"
       b"            limit_conn one 1;\n"
       b"            limit_rate 1024k;\n"
       b"            alias /mnt/sda/;\n"
       b"        }")
DST = (b"location /downloadfile/ {\n"
       b"           #internal;\n"
       b"            limit_conn one 1;\n"
       b"            limit_rate 0;    \n"
       b"            alias /mnt/sda/;\n"
       b"        }")
data = bytearray(open("app_unpacked/device","rb").read())
assert data.count(SRC) == 2, f"expected 2 hits, got {data.count(SRC)}"
open("app_unpacked/device","wb").write(bytes(data).replace(SRC, DST))
print("   device /downloadfile/ unlocked")
PY

echo "==> 5) Patch router bitrate cap instruction"
python3 - <<PY
PATCH_OFFSET = 0x6351c
SRC_BYTES = bytes.fromhex("0b008652")   # mov w11, #0x3000
inst = 0x5280000B | ($KBPS << 5)
DST_BYTES = bytes([inst & 0xff, (inst>>8)&0xff, (inst>>16)&0xff, (inst>>24)&0xff])
data = bytearray(open("app_unpacked/router","rb").read())
actual = bytes(data[PATCH_OFFSET:PATCH_OFFSET+4])
assert actual == SRC_BYTES, f"router[{hex(PATCH_OFFSET)}] mismatch: got {actual.hex()}"
data[PATCH_OFFSET:PATCH_OFFSET+4] = DST_BYTES
open("app_unpacked/router","wb").write(bytes(data))
print(f"   router bitrate cap: {SRC_BYTES.hex()} -> {DST_BYTES.hex()} ({$KBPS} kbps)")
PY

echo "==> 6) Insert the record-at-home gate into start_app (baked home MACs)"
python3 - <<PY
gate = open("$GATE_TEMPLATE").read().replace("%%HOME_MACS%%", "$HOME_MACS_LC")
sa = "rootfs_unpacked/etc/init.d/start_app"
text = open(sa).read()
needle = "./recorder &"
n = text.count(needle)
assert n == 1, f"expected exactly one '{needle}' in start_app, got {n} (firmware layout changed?)"
if not gate.endswith("\n"):
    gate += "\n"
text = text.replace(needle + "\n", gate, 1)
with open(sa, "w", newline="\n") as f:
    f.write(text)
print(f"   start_app recorder launch gated; baked home MACs: $HOME_MACS_LC")
PY

echo "==> 7) Repack app squashfs"
mksquashfs app_unpacked app_new.bin \
    -comp xz -b 262144 -noappend -no-progress \
    -no-exports -all-root -mkfs-time 0 -all-time 0 \
    >/dev/null

echo "==> 8) Repack rootfs squashfs"
mksquashfs rootfs_unpacked rootfs_new.bin \
    -comp xz -b 262144 -noappend -no-progress \
    -no-exports -all-root -mkfs-time 0 -all-time 0 \
    >/dev/null

echo "==> 9) Repack pak (replace BOTH app and rootfs sections)"
PYTHONPATH="$PAK_DIR" python3 - <<PY
from pak_repack import repack
swaps = {
    "rootfs": open("rootfs_new.bin", "rb").read(),
    "app":    open("app_new.bin",    "rb").read(),
}
crc, size, secs = repack("$STOCK", "$OUT", swaps=swaps)
print(f"wrote $OUT  size={size}  crc=0x{crc:08x}")
for name, off, sz in secs:
    marker = "  (replaced)" if name in swaps else ""
    print(f"  {name:10s} start=0x{off:08x} size=0x{sz:08x}{marker}")
PY

echo "==> 10) Verify CRC"
python3 "$PAK_DIR/reolink_crc.py" compute "$OUT"

echo
echo "====================================================================="
echo " Build complete: $OUT"
echo "  - HTTP /downloadfile/ unlock:   carried"
echo "  - Main-stream max bitrate:       $KBPS kbps (was 12288)"
echo "  - start_app record-at-home gate: installed"
echo "      home MACs (suppress rec):  $HOME_MACS_LC"
echo
echo " Boot behavior:"
echo "  - Home gateway MAC matched -> recorder NOT started (no boot stub)."
echo "  - Anything else (field network / no network / error / timeout) -> records."
echo
echo " UNVERIFIED — bench-test the fail-enabled matrix in"
echo " docs/RECORD_GATE_DESIGN.md before any field use, then flash via the"
echo " camera web UI (Settings -> Maintenance -> Local Upgrade)."
echo "====================================================================="
